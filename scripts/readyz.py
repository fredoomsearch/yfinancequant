...
from ops.health import build_readyz_report


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Report service readiness from persisted run artifacts")
...
def _render_text(report: dict) -> str:
    lines = [
        f"Readyz: ok={report.get('ok')} status={report.get('status')}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build_readyz_report(args.artifact_root, args.run_id).model_dump()
    if args.json:
