...
def _build_fake_modeling(tmp_path: Path) -> ModelingResult:
    ...
    return ModelingResult(
        ...
        rationale="Synthetic modeling result for dashboard coverage.",
...
class OpsDashboardTest(unittest.TestCase):
    def test_dashboard_builds_summary_and_html(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dashboard = build_operations_dashboard(str(tmp_path / "artifacts"), result.run_id, limit=10)
            html = render_operations_dashboard_html(dashboard)

            self.assertEqual(dashboard.latest_run_id, result.run_id)
            self.assertEqual(dashboard.readyz.status, "ready")
            self.assertEqual(dashboard.release_gate.status, "pending_soak")
            self.assertGreaterEqual(dashboard.release_board.total_runs, 1)
            self.assertIn("IAC Operations Dashboard", html)
            self.assertIn(result.run_id, html)
            self.assertIn("Latest Run Observability", html)

    def test_dashboard_can_be_persisted_under_run_ops_directory(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            request = PipelineRequest(
                tickers=["AAPL"],
            ...
            dashboard = persist_operations_dashboard(str(tmp_path / "artifacts"), result.run_id, limit=10)
            verify_report = build_run_verification_report(str(tmp_path / "artifacts"), result.run_id)

            self.assertEqual(dashboard.latest_run_id, result.run_id)
            self.assertIn("json", dashboard.artifacts)
            self.assertIn("html", dashboard.artifacts)
            self.assertTrue(Path(dashboard.artifacts["json"].path).exists())
            self.assertTrue(Path(dashboard.artifacts["html"].path).exists())
            self.assertTrue(verify_report["artifacts"]["operations_dashboard"]["exists"])
            self.assertTrue(verify_report["artifacts"]["operations_dashboard_html"]["exists"])
