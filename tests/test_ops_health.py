...
def _build_fake_modeling(tmp_path: Path) -> ModelingResult:
    ...
    return ModelingResult(
        ...
        rationale="Synthetic modeling result for operational readiness coverage.",
...
class OpsHealthTest(unittest.TestCase):
    def test_readyz_and_release_gate_reports_reflect_latest_run(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            readyz = build_readyz_report(str(tmp_path / "artifacts"), result.run_id)
            release_gate = build_release_gate_report(str(tmp_path / "artifacts"), result.run_id)

            self.assertEqual(verify_report["run_id"], result.run_id)
            self.assertTrue(verify_report["checks"]["operations_verify_ok"])
            self.assertTrue(readyz.ok)
            self.assertEqual(readyz.status, "ready")
            self.assertEqual(readyz.latest_run_id, result.run_id)
            self.assertFalse(release_gate.ok)
            self.assertEqual(release_gate.status, "pending_soak")
            self.assertEqual(release_gate.latest_run_id, result.run_id)
            self.assertTrue(release_gate.checks["core_stages_completed"])
            self.assertTrue(release_gate.checks["failed_log_count_zero"])
            self.assertTrue(release_gate.checks["feature_count_minimum"])
            self.assertEqual(release_gate.details["observability"]["stage_statuses"]["orchestrator"], "completed")

    def test_readyz_reports_not_ready_without_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            report = build_readyz_report(str(Path(tmpdir) / "missing_artifacts"))
            release_gate = build_release_gate_report(str(Path(tmpdir) / "missing_artifacts"))
    ...
    def test_release_board_summarizes_recent_runs(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            request = PipelineRequest(
                tickers=["AAPL"],
            ...
            board = build_release_board(str(tmp_path / "artifacts"), limit=10)

            self.assertEqual(board.total_runs, 2)
            self.assertEqual(board.latest_run_id, second.run_id)
            self.assertEqual(board.entries[0].run_id, second.run_id)
            self.assertEqual(board.entries[1].run_id, first.run_id)

    def test_release_gate_blocks_promotion_ready_candidate_without_manual_signoff(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            request = PipelineRequest(
                tickers=["AAPL"],
            ...
            self.assertFalse(release_gate.ok)
            self.assertEqual(release_gate.status, "blocked")
            self.assertFalse(release_gate.checks["adaptive_review_signed_if_required"])

    def test_release_gate_blocks_approved_candidate_without_prepared_application(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            request = PipelineRequest(
                tickers=["AAPL"],
            ...
            self.assertFalse(release_gate.ok)
            self.assertEqual(release_gate.status, "blocked")
            self.assertFalse(release_gate.checks["adaptive_application_prepared_if_reviewed"])
