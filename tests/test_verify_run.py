...
from scripts.verify_run import build_verification_report
from scripts.observability_report import build_observability_report
...
def _build_fake_modeling(tmp_path: Path) -> ModelingResult:
    ...
    return ModelingResult(
        ...
        rationale="Synthetic modeling result for verification script coverage.",
...
class VerifyRunScriptTest(unittest.TestCase):
    def test_build_verification_report_reads_adaptive_and_operations_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            report = build_verification_report(str(tmp_path / "artifacts"), result.run_id)

            self.assertEqual(report["run_id"], result.run_id)
            self.assertEqual(report["status"], RunState.succeeded.value)
            self.assertTrue(report["checks"]["operations_verify_ok"])
            self.assertTrue(report["all_required_artifacts_present"])
            self.assertEqual(report["adaptive"]["mode"], "observe_only")
            self.assertEqual(report["adaptive"]["approval_status"], "observe_only")
            self.assertEqual(report["operations"]["release_stage"], "ops_pending")
            self.assertEqual(report["artifacts"]["shadow_run"]["exists"], False)
            self.assertTrue(report["artifacts"]["adaptive_approval"]["exists"])
            self.assertIn("operations_report_generated", report["log_messages"])

    def test_build_verification_report_uses_latest_run_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            request = PipelineRequest(
                tickers=["AAPL"],
            ...
            report = build_verification_report(str(tmp_path / "artifacts"))

            self.assertEqual(report["run_id"], result.run_id)

    def test_build_observability_report_reads_runtime_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            request = PipelineRequest(
                tickers=["AAPL"],
            ...
            report = build_observability_report(str(tmp_path / "artifacts"), result.run_id)

            self.assertEqual(report["run_id"], result.run_id)
            observability = report["observability"]
            self.assertEqual(observability["stage_statuses"]["orchestrator"], "completed")
            self.assertEqual(observability["data_profile"]["feature_count"], 14)
            self.assertEqual(observability["decision_profile"]["selected_model"], "ensemble")
